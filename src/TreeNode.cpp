...
void TreeNode::display(QApplication* app, std::function<void()> onBack){

    int row = treeHeight(this);
    int column = treeWidth(this);

    window = new mainWindow(800, 800, app, "Binary Tree");
...
//        Line * line = new Line(window->getParentWindow(),x1,y1,x2,y2);
    ...
    std::cout<<(low+(low+high)/2)/2<<" "<<depth+1<<" "<<layout->getCellCenter(depth+1,(low+(low+high)/2)/2)->first<<" ";
